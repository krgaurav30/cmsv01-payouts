...
export class NotificationsService {
    ...
    async listNotifications(recipientUserId) {
        const result = await this.db.query(`select notification_id, corporate_tenant_id, corporate_id, recipient_user_id, title,
       ...
       limit 50`, [recipientUserId]);
        return result.rows.map(mapNotificationRow);
    }
    async markNotificationRead(notificationId, payload) {
       ...
       returning notification_id, corporate_tenant_id, corporate_id, recipient_user_id, title,
                 message, target_section, entity_type, entity_id, read_at, created_at`, [notificationId, payload.actedByUserId]);
        return result.rows[0] ? mapNotificationRow(result.rows[0]) : null;
    }
    async markAllRead(actedByUserId) {
        await this.db.query(`update notifications
       set read_at = coalesce(read_at, now())
       where recipient_user_id = $1
         and read_at is null`, [actedByUserId]);
    }
    ...
    async getUserIdsForPermission(corporateTenantId, permission) {
        const result = await this.db.query(`select cu.user_id
       ...
       where cu.corporate_tenant_id = $1
         ...
         and $2 = any(coalesce(cr.permissions, array[]::text[]))`, [corporateTenantId, permission]);
        return result.rows.map((row) => row.user_id);
    }
    async notifyPermissionRecipients(payload) {
        const recipients = await this.getUserIdsForPermission(payload.corporateTenantId, payload.permission);
