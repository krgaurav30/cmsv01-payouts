...
export class TenantManagementService {
    ...
    async listBankTenants() {
        const result = await this.db.query(`select tenant_id, name, subdomain, primary_color, contact_email, status
       from bank_tenants
       order by tenant_id`);
    ...
    async createBankTenant(payload) {
        const result = await this.db.query(`insert into bank_tenants (tenant_id, name, subdomain, primary_color, contact_email, status)
    ...
    async listCorporateTenants(bankTenantId, status) {
        const clauses = [];
        const values = [];
        if (bankTenantId) {
        ...
        const result = await this.db.query(`select tenant_id, bank_tenant_id, name, legal_entity_name, corporate_admin_email, status
       from corporate_tenants
       ${whereClause}
       order by tenant_id`, values);
        return result.rows.map((row) => ({
            tenantId: row.tenant_id,
    ...
    async getCorporateTenant(tenantId) {
        const result = await this.db.query(`select tenant_id, bank_tenant_id, name, legal_entity_name, corporate_admin_email, status
       from corporate_tenants
       where tenant_id = $1`, [tenantId]);
        const row = result.rows[0];
    ...
    async listCorporates(corporateTenantId, status) {
        const clauses = [];
        const values = [];
        ...
        const whereClause = clauses.length > 0 ? `where ${clauses.join(" and ")}` : "";
        const result = await this.db.query(`select corporate_id, corporate_tenant_id, bank_tenant_id, name, legal_entity_name,
              corporate_admin_email, status
       from corporates
       ${whereClause}
       order by corporate_id`, values);
        return result.rows.map((row) => ({
    ...
    async getCorporate(corporateId) {
        const result = await this.db.query(`select corporate_id, corporate_tenant_id, bank_tenant_id, name, legal_entity_name,
              corporate_admin_email, status
       from corporates
       where corporate_id = $1`, [corporateId]);
        const row = result.rows[0];
        if (!row) {
            return null;
        }
    ...
    async createCorporateTenant(payload, status = "active") {
        ...
        const result = await this.db.query(`insert into corporate_tenants (
    ...
    async createCorporate(payload, status = "active") {
        const linkedBank = await this.getBankTenant(payload.bankTenantId);
        ...
        const result = await this.db.query(`insert into corporates (
