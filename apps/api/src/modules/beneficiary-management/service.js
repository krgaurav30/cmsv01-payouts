...
export class BeneficiaryManagementService {
    ...
    constructor(tenantManagementService = new TenantManagementService(), identityAccessService = new IdentityAccessService(loadConfig()), notificationsService = new NotificationsService()) {
    ...
    async listBeneficiaries(filters) {
        ...
        const result = await this.db.query(`select beneficiary_id, bank_tenant_id, corporate_tenant_id, corporate_id, name,
              account_number, ifsc, phone_number, category, tags, status,
              approval_state, review_comment, updated_at
       from beneficiaries
       ${whereClause}
       order by updated_at desc nulls last, beneficiary_id desc`, values);
    ...
    async getBeneficiary(beneficiaryId) {
        const result = await this.db.query(`select beneficiary_id, bank_tenant_id, corporate_tenant_id, corporate_id, name,
              ...
              approval_state, review_comment, updated_at
       from beneficiaries
       where beneficiary_id = $1`, [beneficiaryId]);
    ...
    async createBeneficiary(payload) {
        ...
        if (!actor ||
            !(await this.identityAccessService.userHasPermission(payload.createdByUserId, "beneficiary.make"))) {
            return {
                error: "forbidden"
            };
        }
        ...
        const duplicate = await this.db.query(`select beneficiary_id
       from beneficiaries
       where corporate_id = $1
         and lower(name) = lower($2)
         and account_number = $3
       limit 1`, [payload.corporateId, payload.name, payload.accountNumber]);
        if (duplicate.rows[0]) {
            return {
        ...
        const result = await this.db.query(`insert into beneficiaries (
    ...
    async createPublishedBeneficiary(payload) {
    ...
    async authorizePublishedBeneficiary(beneficiaryId, payload) {
        const actor = await this.identityAccessService.getCorporateUserByUsername(payload.actorUsername);
        if (!actor) {
            return {
                error: "actor_not_found"
    ...
    async applyApprovalAction(beneficiaryId, payload) {
        const actor = await this.identityAccessService.getCorporateUserById(payload.actedByUserId);
        if (!actor ||
            !(await this.identityAccessService.userHasPermission(payload.actedByUserId, "beneficiary.checker"))) {
            return {
                error: "forbidden"
            };
        }
        ...
        const result = await this.db.query(`update beneficiaries
       ...
       returning beneficiary_id, bank_tenant_id, corporate_tenant_id, corporate_id, name,
                 account_number, ifsc, phone_number, category, tags, status,
                 approval_state, review_comment, updated_at`, [beneficiaryId, nextApprovalState, nextStatus, payload.comment ?? null]);
    ...
    async applyStatusAction(beneficiaryId, payload) {
        const actor = await this.identityAccessService.getCorporateUserById(payload.actedByUserId);
        if (!actor ||
            !(await this.identityAccessService.userHasPermission(payload.actedByUserId, "beneficiary.make"))) {
            return {
                error: "forbidden"
            };
        }
       ...
       returning beneficiary_id, bank_tenant_id, corporate_tenant_id, corporate_id, name,
                 account_number, ifsc, phone_number, category, tags, status,
                 approval_state, review_comment, updated_at`, [beneficiaryId, nextStatus, payload.comment ?? null]);
        return {
            data: mapBeneficiaryRow(result.rows[0])
        };
    }
    async updateBeneficiary(beneficiaryId, payload) {
        const existing = await this.getBeneficiary(beneficiaryId);
        if (!existing) {
            return {
                error: "beneficiary_not_found"
    ...
    async deleteBeneficiary(beneficiaryId) {
        const result = await this.db.query(`delete from beneficiaries where beneficiary_id = $1`, [beneficiaryId]);
        return {
            deleted: (result.rowCount ?? 0) > 0
        };
    }
    async generateBeneficiaryId() {
        for (let attempt = 0; attempt < 5; attempt += 1) {
            const candidate = `${Date.now()}`.slice(-6) + `${Math.floor(Math.random() * 10000)}`.padStart(4, "0");
            const existing = await this.db.query(`select beneficiary_id from beneficiaries where beneficiary_id = $1 limit 1`, [candidate]);
            if (!existing.rows[0]) {
                return candidate;
            }
        }
...
function inferBankNameFromIfsc(ifsc) {
