...
export class ApprovalMatrixManagementService {
  ...
  async buildTransactionApprovalPlan(corporateTenantId: string, amount: number) {
    const result = await this.db.query<ApprovalMatrixRow>(
      `select matrix_id, corporate_tenant_id, entity_type, amount_from, amount_to,
              approval_levels, roles, status, created_by_user_id, created_by_role,
              created_at, updated_at
      ...
      [corporateTenantId, amount]
    );

    const matrices = result.rows.map(mapApprovalMatrixRow);
    if (matrices.length === 0) {
      const fallbackRoles = await this.identityAccessService.listApprovedTransactionCheckerRoleNames(
        corporateTenantId
      );
